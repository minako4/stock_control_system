...
public class LoginFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
        ...
        if (servletPath.matches("/css.*")) {
            ...
            //セッションからログインしている店舗の情報を取得
            StoreView sv = (StoreView) session.getAttribute(AttributeConst.LOGIN_STORE.getValue());

            if (sv == null) {

                if (!(ForwardConst.ACT_LOGIN.getValue().equals(action)
                    ...
                    ((HttpServletResponse) response).sendRedirect(
                            contextPath
                                    + "?action=" + ForwardConst.ACT_LOGIN.getValue()
